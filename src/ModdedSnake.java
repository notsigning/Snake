...
public class ModdedSnake extends JComponent{
    ...
    private int foodx;
    private int foody;
    //dimensions of the board; may be modified
    private final int width = 20;
    private final int height = 15;
    Color currentColor;
    Color foodColor; //FDA disapproved colorings lol
    private final Color[] COLOR_LIST = {Color.BLACK, Color.BLUE, Color.CYAN,
    ...
    public ModdedSnake(){ //constructor
        super();
        reset();
    }
    public void reset(){ //resets the game
        ...
        foody = 7;
        score = 0;
        currentColor = new Color(75,75,255);
        foodColor = new Color(255,75,75);
    }
    ...
    */
    public boolean move(){
        ...
        if(!(foodx == xCoords.get(xCoords.size()-1) && foody == yCoords.get(yCoords.size()-1))) {
            ...
            while(xCoords.indexOf(foodx) > 0 && yCoords.indexOf(foody) > 0) {
                //regenerate again if there is overlap
                foodx = (int)(Math.random()*width);
                foody = (int)(Math.random()*height);
            }
            currentColor = foodColor;
            foodColor = COLOR_LIST[(int)(Math.random()*12)];
            score++;
        }
        ...
        if(width <= xCoords.get(xCoords.size()-1) || xCoords.get(xCoords.size()-1) < 0){ //out of bounds - x
            return false;
        }
        else if(height <= yCoords.get(yCoords.size()-1) || yCoords.get(yCoords.size()-1) < 0){  //out of bounds - y
            return false;
        }
        return true;
    }
